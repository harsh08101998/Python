...
conn3=mysql.connector.connect(host='10.1.22.138', user='postern', password='spyonme', database='campaignix 
...
for accountsid in listt_api:
    print(accountsid)
    query6="select sid from campaigns where send_at >='{} 00:00:00' and send_at <='{} 23:59:59' and account_sid='{}'".format(startdate,lastdate,accountsid)
    my_cursor3.execute(query6)
    data2=my_cursor3.fetchall()
    countt=0
    complete_count=0
    for i in data2:
        query666="select count(*) from calls where campaign_sid='{}'".format(i[0])
        my_cursor3.execute(query666)
        dataa=my_cursor3.fetchall()
        countt+=int(dataa[0][0])

        query77="select count(*) from calls where campaign_sid='{}' and calls.status='completed'".format(i[0])
        print(query77)
        my_cursor3.execute(query77)
        dataa7=my_cursor3.fetchall()
        complete_count+=int(dataa7[0][0])

    print(countt)
    print(complete_count)
    total_calls[i] = countt
    total_completed_calls[i] = complete_count

# # ############  Total Number of Completed Calls per tenant through Campaigns in API.  ################
#     for i in data2:
#         query666="select count(*) from calls where campaign_sid='{}' and calls.status='completed'".format(i[0])
#         my_cursor3.execute(query666)
#         dataa=my_cursor3.fetchall()
#         countt+=int(dataa[0][0])
#     print(countt)
#     total_calls_count = countt
