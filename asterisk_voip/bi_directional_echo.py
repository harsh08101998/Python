...
async def echo(ws):
    ...
    while not ws.closed:
        ...
        if data['event'] == "start":
            ...
            if not has_started_uni and args.stream_type == "unidirectional":
                service = google_doc_helper.authenticate('DRIVE')
                google_doc_helper.create_new_doc(service, data['start']['call_sid'])
                t1 = threading.Thread(target=stream_transcript, args=(ws, data['start']['call_sid']))
                t1.daemon = True
                t1.start()
                has_started_uni = True
        if data['event'] == "media":
...
def stream_transcript(ws, call_sid):
    service = google_doc_helper.authenticate('DRIVE')
